...
CREATE TABLE IF NOT EXISTS leads(
  ...
  docCliente TEXT, endCliente TEXT, mercado TEXT DEFAULT 'BR',
  atualizado TEXT DEFAULT (datetime('now','localtime')));
```

Status: `novo | redesenhado | publicado | proposta | respondeu | fechado | descartado`. `slug` é a chave. `mercado` (`BR`/`PT`) vem do lote de `/prospectar` — guarde por lead para o `/contrato` saber se pede CPF/CNPJ ou NIF, e para o comparador/kanban poder mostrar a bandeira/cidade certa.

## Moeda e preço de referência no dashboard

Ao regravar o snapshot, inclua no JSON embutido os campos `moeda`, `localeNum` e `precoPagina` lidos de `config.mercados[padrao]` (ex.: `{"moeda":"R$","localeNum":"pt-BR","precoPagina":700}` para BR, ou `{"moeda":"€","localeNum":"pt-PT","precoPagina":200}` para PT). O template usa esses três campos para formatar TODOS os valores financeiros — nunca hardcode "R$" ao gerar o snapshot. **Limitação conhecida**: se o usuário prospecta BR e PT ao mesmo tempo, os agregados financeiros (Receita, Potencial, Financeiro) somam tudo na moeda do mercado padrão — é uma aproximação aceitável para uso solo; não tente converter câmbio.
