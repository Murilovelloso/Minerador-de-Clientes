...
1. **Banco (`prospector.db`)**: nome do cliente, cidade, valor fechado, URL publicada, `mercado` do lead (BR/PT).
2. **Config (`prospector-config.json`)**: dados do PRESTADOR — nome, documento, endereço, cidade/UF (campo `contratante`; se não existir, colete do usuário UMA vez e salve).
3. **Usuário** (ele pergunta ao cliente): documento e endereço do CONTRATANTE, forma de pagamento, prazo, manutenção mensal (sim/não + valor).

**Documento por mercado** (o template já é parametrizado via `{{..._LABEL}}`, só preencher certo):
- Lead `mercado='BR'`: `CPF_CNPJ_..._LABEL` = "inscrito(a) no CPF" ou "inscrita no CNPJ".
- Lead `mercado='PT'`: use `CPF_CNPJ_..._LABEL` = "portador(a) do NIF" e o valor é o NIF (9 dígitos), sem distinção pessoa física/jurídica.
