...
**WHATSAPP: capture SEMPRE, separado do telefone.** Fontes, na ordem: botão/link de WhatsApp no site do lead (procure `wa.me/`, `api.whatsapp.com` ou ícone de WhatsApp — extraia o número do link); telefone celular do perfil do Maps. Registre SEMPRE em formato internacional completo, pronto pra `wa.me` — o formato depende do `mercado` da busca (config `prospector-config.json` → `mercados[].codigoPais`):

- **Brasil (mercado BR)**: `55 + DDD + número` (ex.: `5511999990000`). Números com 9º dígito são celular — assuma WhatsApp.
- **Portugal (mercado PT)**: `351 + número` (ex.: `351912345678`). Não existe DDD — o número português tem 9 dígitos e celular começa com `9`.

Se o mercado da busca atual não estiver claro, confirme com o usuário antes de gravar o número (o prefixo errado quebra o link `wa.me`). O WhatsApp alimenta os botões do dashboard e o plano B de abordagem quando o e-mail não responde.
