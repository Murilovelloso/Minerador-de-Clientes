...
2. **Determine o mercado**: se `config.mercados` tiver mais de um país configurado (ex.: BR e PT), pergunte qual usar nesta busca (ou aceite `$ARGUMENTS` explícito, ex. "nutricionistas Lisboa PT"). Se só houver um mercado configurado, use-o direto. O mercado escolhido define `codigoPais`, `moeda` e `documento` para o resto do fluxo deste lote — registre-o em `leads.md`/banco por lead (coluna `mercado`) para os comandos seguintes (`/redesenhar`, `/publicar`, `/proposta`, `/contrato`) usarem o formato certo sem perguntar de novo.
3. Determine nicho e cidade dentro do mercado escolhido: use os argumentos `$ARGUMENTS` se informados; senão, pergunte ao usuário qual dos nichos padrão do config usar (e confirme a cidade daquele mercado). O usuário SEMPRE pode trocar nicho e cidade na hora — nunca trave nos padrões.
...
- Coletar: nome, nota, nº de avaliações, telefone, **WhatsApp no formato internacional do mercado da busca** (BR: `55+DDD+número`; PT: `351+número` — ver skill), e-mail, URL do site e o motivo objetivo pelo qual o site é ruim
